...
int get_student(int fd, int id, student_t *s) {
    if (id < MIN_STD_ID || id > MAX_STD_ID) {
        return SRCH_NOT_FOUND;  // Invalid ID
    }
    ...
    if (memcmp(s, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) == 0) {
        return SRCH_NOT_FOUND;  // Empty record means student doesn't exist
    }
...
int add_student(int fd, int id, char *fname, char *lname, int gpa) {
    ...
    new_student.fname[sizeof(new_student.fname) - 1] = '\0'; // Ensure null termination
...
int print_db(int fd) {
    ...
    while (read(fd, &student, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
            ...
            float real_gpa = student.gpa / 100.0;  // Convert integer GPA to float
...
void print_student(student_t *s) {
    ...
    printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST NAME", "LAST_NAME", "GPA");
    float real_gpa = s->gpa / 100.0;  // Convert integer GPA to float
...
int compress_db(int fd) {
    ...
    new_fd = open(TMP_DB_FILE, O_RDWR | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
    if (new_fd == -1) {
        printf(M_ERR_DB_OPEN);
        return ERR_DB_FILE;
    }

    // Seek to the beginning of the database file
    if (lseek(fd, 0, SEEK_SET) == -1) {
        printf(M_ERR_DB_READ);
        close(new_fd);
        return ERR_DB_FILE;
    ...
    // Read through the existing database and copy valid records
    while (read(fd, &student, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
    ...
    // Rename temporary database file to the original database file
